...
Func Parent()
     ...
     GUICtrlSetTip($sensX,'Horizontal scrolling sensitivity. Set a negative number for reverse scrolling.' & @CRLF & 'NOTE: Chrome do not support simultaneous X+Y scrolling, whereas Firefox works flawlessly.')
...
Func ScrollMouseXY($dx,$dy)
     ...
     Local Static $xData = DllStructCreate('dword type;struct;long;long;dword data;dword flag;dword;ulong_ptr;endstruct;', $ptr)
     Local Static $yData = DllStructCreate('dword type;struct;long;long;dword data;dword flag;dword;ulong_ptr;endstruct;', $ptr+$SIZE)
     Local Static $_ = SetupScrollData($xData,$yData)
     Local $count = ($dx?1:0)+($dy?1:0)
     Local $addr = $dx ? $ptr : $ptr+$SIZE
